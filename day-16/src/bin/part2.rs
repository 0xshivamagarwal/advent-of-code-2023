...
fn process(input: &str) -> usize {
    ...
    (0..n).fold(0, |max, i| {
        let ans = [
            solve(&input, &map, [(0, i, Direction::DOWN)]),
            solve(&input, &map, [(i, 0, Direction::RIGHT)]),
            solve(&input, &map, [(n - 1, i, Direction::UP)]),
            solve(&input, &map, [(i, n - 1, Direction::LEFT)]),
        ];

        max.max(*ans.iter().max().unwrap())
    })
}
...
..//.|...."
    ...
    #[test]
    fn it_works_2() {
        assert_eq!(process(include_str!("../../input.txt")), 7853);
    }
}
